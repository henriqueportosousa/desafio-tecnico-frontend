...
function ListarChamado({ chamados, aoExcluir }) {
    ...
    return (
        <main className="pagina">
            ...
            <ul className="lista">
                {chamados?.length === 0 ? (
                    <li className="item-vazio">Nenhum chamado cadastrado.</li>
                ) : (
                    chamados.map((chamado) => (
                        <li key={chamado.id}>
                            <strong>Título: {chamado.titulo}</strong>
                            ...
                            <div className="acoes">
                                <Link to={`/chamados/editar/${chamado.id}`} className="botao-alterar">
                                    Alterar
                                </Link>
                                <button onClick={() => confirmarExclusao(chamado)} className="botao-excluir">
                                    Excluir
                                </button>
                            </div>
                        </li>
